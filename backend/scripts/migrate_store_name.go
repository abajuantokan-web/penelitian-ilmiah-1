...
// migrate_store_name.go — One-time data migration
// Copies store_name from users.store_name → seller_profiles.store_name
// for all sellers who either:
//   a) have no seller_profile row yet (creates one), or
//   b) have a seller_profile but store_name is empty (updates it).
//
// Usage (run from the /backend directory):
//
//	go run scripts/migrate_store_name.go
package main
...
func main() {
	...
	for _, seller := range sellers {
		...
		if err != nil {
			...
			// Profile exists but store_name is empty → update
